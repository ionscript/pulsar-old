...
class BannerController extends Controller
{
    ...
    {
        $this->language->load('extension/module/banner');

        $this->document->setTitle($this->language->get('heading_title'));

        if ($this->request->isPost() && $this->validate()) {
            if (!$this->request->hasQuery('id')) {
                $this->model('setting/module')->addModule('banner', $this->request->getPost());
            } else {
        ...
        if (!$this->request->hasQuery('id')) {
            $data['action'] = $this->url->link('extension/module/banner', 'token=' . $this->session->get('token'));
        } else {
            $data['action'] = $this->url->link('extension/module/banner', 'token=' . $this->session->get('token') . '&id=' . $this->request->getQuery('id'));
        }
        ...
        if ($this->request->hasPost('module_banner_status')) {
            $data['module_banner_status'] = $this->request->getPost('module_banner_status');
        } elseif (!empty($module_info)) {
            $data['module_banner_status'] = $module_info['module_banner_status'];
        } else {
            $data['module_banner_status'] = '';
        }

        $data['header'] = $this->controller('header');
        $data['footer'] = $this->controller('footer');

        $this->response->setContent($this->view('extension/module/banner', $data));
    }
